...
readsb-prod-0 readsb-prod-1 readsb-staging-0 https://github.com/adsblol/globe_history The data is made available by GitHub releases, a release is available daily for each of the above instances.
...
Usage # https://api.adsb.lol `}),e.add({id:3,href:"/website/docs/get-started/introduction/",title:"Feeding",description:`Feed ADSB.lol # Adding a receiver to our network is easy. You can use any of the following methods:
...
https://mlat.adsb.lol Please note that this map respects the privacy flag used by feeders. So any site that feeds with privacy enabled will not be shown on this map.
It was made by lemonodor and wiedehopf `}),e.add({id:8,href:"/website/docs/feeders-only/my-map/",title:"My Map",description:"My Map is a map of the aircrafts that your station is receiving.",content:`Your map is available at the following address: https://my.adsb.lol This link, accessed by your IP, redirects you to your own subdomain that can be shared with others publicly.
...
Please follow the detailed instructions on the ADSB.im website.
`}),e.add({id:10,href:"/website/docs/get-started/docker/",title:"Docker Toolkit",description:`By default, it feeds MLAT+ADSB to adsb.lol. You can enable UAT/ACARS/VDL2, and feed to your plane data to FlightRadar24, Radarbox, Piaware, and more It is designed to be run on a Raspberry Pi, but can be run on any Linux Debian-like system.
...
`,content:`By default, it feeds MLAT+ADSB to adsb.lol. You can enable UAT/ACARS/VDL2, and feed to your plane data to FlightRadar24, Radarbox, Piaware, and more It is designed to be run on a Raspberry Pi, but can be run on any Linux Debian-like system.
With a few commands , you can easily feed to other community aggregators.
Quick Start with Docker # To get started with the docker client,
Run this as root on a fresh install of Raspberry Pi OS Lite or similar.
...
curl -Ls https://raw.githubusercontent.com/adsblol/feed/main/bin/adsblol-init | bash cd /opt/adsblol/ cp .env.example .env Then, set the environment variables.
...
http://IP:8080 (readsb) http://IP:8082 (adsblol) https://adsb.lol (ADSB) https://mlat.adsb.lol (MLAT) Usage # By default, the client will feed to adsb.lol.
...
I cannot find myself on the MLAT Map # adsb.lol enables the --privacy flag for your MLAT client by default. This hides you from the MLAT map.
...
Feeding directly to other aggregators # Where possible, adsb.lol commits to share data and ingest data directly with other aggregators which are willing to license their data openly.
...
TheAirTraffic.com is run by Jack Sweeney This is not an endorsement and adsb.lol/myself are not affiliated with these aggregators.
Run # NOTE: This is using --privacy, which excludes you from adsb.lol map, and should exclude you from other aggregators maps too.
...
adsblol-env set ADSBLOL_ADDITIONAL_NET_CONNECTOR &quot;feed.adsbexchange.com,30004,beast_reduce_out;feed.theairtraffic.com,30004,beast_reduce_out&quot; adsblol-env set ADSBLOL_ADDITIONAL_MLAT_CONFIG &quot;feed.adsbexchange.com,31090,39001;feed.theairtraffic.com,31090,39002&quot; adsblol-env set MLATHUB_NET_CONNECTOR &quot;adsblol,39000,beast_in;adsblol,39001,beast_in;adsblol,39002,beast_in&quot; `}),e.add({id:11,href:"/website/docs/feeders-only/beast-mlat-out/",title:"BEAST MLAT Out",description:"Our aggregated data is available to all feeders.",content:`License: ODbL 1.0 Hostname: out.adsb.lol
...
readsb --net-only --net-connector=out.adsb.lol,1337,beast_in --net-connector=out.adsb.lol,1338,sbs_in_mlat The typical use case for this would be to do some form of data analysis on the full raw data stream from all the feeders. It could also be used for certain visualization goals of live or past flight activity.
Please make sure that you avoid creating feeding loops - that is why the disclaimer above urges you not to use the readsb in your feeder setup to connect to this stream. A good way to set up a separate environment to start using this data stream would be to spin up a Docker container with its own readsb instance. The necessary docker file to get started with that is included in the readsb implementation that is most commonly used at this point (and that is also the underlying implementation used for ADSB.lol): @wiedehopf&rsquo;s readsb `}),e.add({id:12,href:"/website/docs/feeders-only/re-api/",title:"re-api",description:"re-api is the readsb HTTP API, available to all ADSB.lol feeders.",content:`License: ODbL 1.0 You can access the API at the following address: https://re-api.adsb.lol This is the readsb API of our entire network, unfiltered and unmodified.
...
https://re-api.adsb.lol?circle=52,2,200 `}),e.add({id:13,href:"/website/docs/get-started/bare-metal/",title:"Bare Metal",description:` These scripts aid in setting up your current ADS-B receiver to feed adsb.lol They will not disrupt any existing feed clients already present 1: Find coordinates / elevation: # https://www.freemaptools.com/elevation-finder.htm 2: Install the adsblol feed client # curl -L -o /tmp/lol-feed.sh https://adsb.lol/feed.sh sudo bash /tmp/lol-feed.sh 3: Check if your feed is working # That one&rsquo;s easy! Just go to https://adsb.lol and you should show as feeding.
...
curl -L -o /tmp/lol-update.sh https://raw.githubusercontent.com/adsblol/feed/master/update.sh sudo bash /tmp/lol-update.sh Troubleshooting issues # If you encounter issues, please do a reboot and then supply these logs on zulip (last 20 lines for each is sufficient):
...
By sharing data with ADSB.lol, you agree to license your data as CC0 .
In exchange, you get access to the aggregated data of all the other aggregators, including ADSB.lol, licensed to you as CC0 .
This way, everyone benefits from the aggregated data of everyone else.
Reach out to info [ at ] adsb.lol
...
Run by Jack Sweeney and members of Ground Control `}),e.add({id:16,href:"/website/docs/acknowledgements/open-source/",title:"Open Source",description:`readsb # readsb is an ADS-B decoder swiss knife.
It is used extensively all the way from the feeders to what is being shown on the map.
LICENSE tar1090 # tar1090 provides an improved webinterface for use with ADS-B decoders readsb / dump1090-fa.
LICENSE mlat-server # mlat-server is a Mode S multilateration server that is designed to operate with clients that do not have synchronized clocks.
LICENSE MLAT Map and Sync Table # MLAT Table License `,content:`readsb # readsb is an ADS-B decoder swiss knife.
It is used extensively all the way from the feeders to what is being shown on the map.
LICENSE tar1090 # tar1090 provides an improved webinterface for use with ADS-B decoders readsb / dump1090-fa.
LICENSE mlat-server # mlat-server is a Mode S multilateration server that is designed to operate with clients that do not have synchronized clocks.
LICENSE MLAT Map and Sync Table # MLAT Table License MLAT Map License Virtual Radar Server Standing Data # The VRS standing data is formatted and used for showing routes.
Acknowledgements go to the Virtual Radar Server contributors adsb.lol API # We contribute some code.
The API is possible thanks to Valkey acting as the database for the fun Route, My Map and Screenshot features.
doks # This website was built using Hugo and doks finally # ingress-nginx cert-manager fluxcd haproxy kubernetes grafana mimir loki `}),e.add({id:17,href:"/website/docs/hardware/adsb-equipment/",title:"ADS-B + UAT Equipment",description:"1090MHz and 978MHz equipment.",content:`SDR # AirNav RadarBox FlightStick (filter, no LNA) US DE NL IT UK FR ES (good starter, cheap) RTL-SDR (no filter, supports Bias-T to power other devices, most common! great value.) AliExpress no-name 1090 filtered (cheapest, least recommended) Best: Airspy Mini (no built-in filter, supports Bias-T, usually is best paired with a high quality LNA) Filter # If your SDR doesn&rsquo;t have a filter, you might benefit from one.
...
Nooelec Lana US DE UK NL FR ES IT Antenna # nooelec indoor US PCB antenna (cheapest) Sirio Antenne , based in Italy Vinnant , based in Slovakia (best) DPD Productions , based in US (best) Coax # Vinnant SMA Adapters # AliExpress (Many types!) `}),e.add({id:18,href:"/website/docs/acknowledgements/sponsors/",title:"Sponsors",description:`CloudFlare # As an open source project, we qualify for CloudFlare&rsquo;s free pro plan Zulip # Zulip is an open-source modern team chat app designed to keep both live and asynchronous conversations organized.
...
`}),e.add({id:19,href:"/website/docs/overview/status/",title:"Status",description:"Uptime Status #",content:"Uptime Status # "}),e.add({id:20,href:"/website/docs/open-data/",title:"Open Data",description:"ADSB.lol open data.",content:""}),e.add({id:21,href:"/website/docs/hardware/",title:"Hardware",description:"Recommended hardware",content:""}),e.add({id:22,href:"/website/docs/acknowledgements/",title:"Acknowledgements",description:"ADSB.lol acknowledgements.",content:""}),e.add({id:23,href:"/website/docs/",title:"Docs",description:"ADSB.lol docs.",content:""}),search.addEventListener("input",t,!0);function t(){const s=5;var n=this.value,o=e.search(n,{limit:s,enrich:!0});const t=new Map;for(const e of o.flatMap(e=>e.result)){if(t.has(e.doc.href))continue;t.set(e.doc.href,e.doc)}if(suggestions.innerHTML="",suggestions.classList.remove("d-none"),t.size===0&&n){const e=document.createElement("div");e.innerHTML=`No results for "<strong>${n}</strong>"`,e.classList.add("suggestion__no-results"),suggestions.appendChild(e);return}for(const[r,a]of t){const n=document.createElement("div");suggestions.appendChild(n);const e=document.createElement("a");e.href=r,n.appendChild(e);const o=document.createElement("span");o.textContent=a.title,o.classList.add("suggestion__title"),e.appendChild(o);const i=document.createElement("span");if(i.textContent=a.description,i.classList.add("suggestion__description"),e.appendChild(i),suggestions.appendChild(n),suggestions.childElementCount==s)break}}})()